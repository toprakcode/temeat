...
export const navItems = [
  ...
  { id: "tables", label: "Masalar", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="21" x2="9" y2="9"/></svg> },
  ...
  { id: "payments", label: "Ödemeler", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg> },
        ...
        {navItems.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className="btn"
              ...
              {item.id === "payments" && (pendingPaymentsCount || 0) > 0 && <span className="sidebar-label" style={{ marginLeft: "auto", fontSize: 10, fontWeight: 800, background: "#a855f7", borderRadius: 6, padding: "2px 7px", color: "#fff", boxShadow: "0 2px 8px rgba(168,85,247,0.4)" }}>{pendingPaymentsCount}</span>}
            </button>
          );
        })}
      </nav>
